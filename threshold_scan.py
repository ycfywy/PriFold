...
def build_dataset(mode, data_dir, max_len, tokenizer):
    """Build test dataset with a given length filter (< max_len)."""
    if mode == 'bprna':
...
def main(args):
    ...
    # Threshold sweep from 0.00 to 1.00 with step 0.01.
    ...
    for thr in thresholds:
        ...
        for probs, label in zip(all_probs, all_labels):
            pred = (probs > thr).astype(np.float32).reshape(-1)
            label_np = label.reshape(-1)
            prec_list.append(precision_score(label_np, pred))
            rec_list.append(recall_score(label_np, pred))
            f1_list.append(f1_score(label_np, pred))
    ...
    print(f"(reference threshold 0.45 result is printed in the sweep table above)")
