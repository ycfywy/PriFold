...
# ============================================================
def compute_record_features(record, max_len=490):
    """Compute structural features for a training record."""
    ct = np.load(record.ct_path).astype(np.float32)
    seq = record.seq.upper().replace('U', 'T')
    length = min(len(seq), max_len)
    ct = ct[:length, :length]

    # Upper triangle, |i-j| >= 3
    ...
    if n_pairs == 0:
        return {'n_pairs': 0, 'density': 0, 'mean_distance': 0, 'distance_hist': [0]*5}

    pair_positions = np.where(ct[:length, :length] * mask > 0.5)
    distances = pair_positions[1] - pair_positions[0]
    ...
    for k in range(len(bins) - 1):
        count = int(((distances >= bins[k]) & (distances < bins[k+1])).sum())
...
def find_hardcase_indices(train_records, badcase_path, similarity_threshold=0.85, logger=None):
    """Find indices of training samples that are structurally similar to test bad cases."""
    # Load bad case features from v10 analysis
    with open(badcase_path) as f:
        test_results = json.load(f)
    bad_cases = [s for s in test_results if s['f1'] < 0.3]
    if logger:
        logger.info(f'[Oversample] {len(bad_cases)} test bad cases loaded')

    # Compute features for all training records
    if logger:
        logger.info(f'[Oversample] Computing features for {len(train_records)} training records...')

    train_features = []
    for i, rec in enumerate(train_records):
        if (i + 1) % 2000 == 0 and logger:
            logger.info(f'  [{i+1}/{len(train_records)}]')
        try:
            feat = compute_record_features(rec)
            feat['index'] = i
            feat['length'] = len(rec.seq)
            feat['family'] = rec.file_name.split('_')[1] if len(rec.file_name.split('_')) >= 3 else 'unknown'
            train_features.append(feat)
        except Exception as e:
            train_features.append({'index': i, 'length': 0, 'n_pairs': 0,
                                   'density': 0, 'mean_distance': 0, 'distance_hist': [0]*5,
                                   'family': 'unknown'})

    # Compute density for bad cases that don't have it
    for bc in bad_cases:
        if 'density' not in bc:
            gt_pairs = bc.get('gt_pairs', 0)
            length = bc.get('length', 1)
            total_possible = length * (length - 1) / 2
            bc['density'] = gt_pairs / max(total_possible, 1)
        if 'mean_distance' not in bc:
            bc['mean_distance'] = bc.get('mean_gt_distance', 0)

    # For each bad case, find similar training samples
    similar_indices = set()
    for bc in bad_cases:
        bc_len = bc['length']
        bc_density = bc.get('density', 0)
        bc_mean_dist = bc.get('mean_distance', 0)
        bc_family = bc.get('family', 'unknown')
        bc_n_pairs = bc.get('gt_pairs', 0)

        for tf in train_features:
            if tf['length'] == 0 or tf['n_pairs'] == 0:
                continue

            # Length similarity
            len_sim = 1.0 - abs(tf['length'] - bc_len) / max(bc_len, tf['length'], 1)

            # Pair count similarity (more direct than density)
            pair_sim = 1.0 - abs(tf['n_pairs'] - bc_n_pairs) / max(bc_n_pairs, tf['n_pairs'], 1)

            # Mean distance similarity
            dist_sim = 1.0 - abs(tf['mean_distance'] - bc_mean_dist) / max(bc_mean_dist, tf['mean_distance'], 1)

            # Family match bonus
            family_bonus = 0.1 if tf['family'] == bc_family else 0.0

            score = 0.30 * len_sim + 0.30 * pair_sim + 0.25 * dist_sim + 0.15 * family_bonus

            if score >= similarity_threshold:
                similar_indices.add(tf['index'])

    if logger:
        logger.info(f'[Oversample] Found {len(similar_indices)} training samples similar to bad cases '
                    f'(threshold={similarity_threshold})')

    return sorted(similar_indices)


def build_oversampled_records(train_records, hardcase_cfg, logger=None):
    """Build oversampled training records list."""
    if not hardcase_cfg.get('enabled', False):
        return train_records

    badcase_path = hardcase_cfg['badcase_analysis_path']
    threshold = hardcase_cfg.get('similarity_threshold', 0.85)
    factor = hardcase_cfg.get('oversample_factor', 2)

    hard_indices = find_hardcase_indices(train_records, badcase_path, threshold, logger)

    # Build new records: original + extra copies of hard cases
    new_records = list(train_records)
    extra = 0
    for idx in hard_indices:
        for _ in range(factor - 1):  # factor=2 means 1 extra copy
            new_records.append(train_records[idx])
            extra += 1

    if logger:
        logger.info(f'[Oversample] Original: {len(train_records)}, Added: {extra}, '
                    f'Total: {len(new_records)} ({extra/len(train_records)*100:.1f}% increase)')

    return new_records


# ============================================================
# Model building (same as v10)
# ============================================================
...
def plot_curves(history, output_dir, logger):
    try:
        ...
        ax = axes[2, 1]
        test_mccs = [h.get('test_mcc') for h in history if 'test_f1' in h]
...
def main():
    ...
    # Data — with hard-case oversampling
    data_dir = config['paths']['data_dir']
    max_len = tcfg.get('max_len_filter', 490)
    train_recs = build_records(data_dir, f'{tcfg["dataset_mode"]}-train', max_len=max_len)

    # Apply hard-case oversampling
    hardcase_cfg = tcfg.get('hardcase_oversample', {})
    train_recs = build_oversampled_records(train_recs, hardcase_cfg, logger)
