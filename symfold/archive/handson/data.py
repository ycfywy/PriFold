...
主要改动 vs `symfold/v1/data.py`:
  - `build_records(stage)` 增加 single-dataset 模式：
        bprna-train, bprna-val, bprna-test
        rnastralign-train, rnastralign-val, rnastralign-test
        archiveii-test
        combined-train, combined-val   (= 旧 'train' / 'val'，向后兼容)
  - 顶层入口 `build_loader(stage, ...)` 不变。
  - `metrics.py` 的 contact_metrics 也共用（v2 直接 import）。
"""
...
def build_records(data_dir: str, stage: str, max_len: int = 490) -> list[RNARecord]:
    ...
    # ----- single-dataset modes (v2 主战场) -----
    if stage == 'bprna-train':
        add_bprna('train', 'TR0')
    elif stage == 'bprna-val':
        add_bprna('val', 'VL0')
    elif stage == 'bprna-test':
        add_bprna('test', 'TS0')
    elif stage == 'rnastralign-train':
        add_rnastr('train', 'tr')
    elif stage == 'rnastralign-val':
        add_rnastr('val', 'vl')
    elif stage == 'rnastralign-test':
        add_rnastr('test', 'ts')
    elif stage == 'archiveii-test':
        add_archive()
    # ----- combined modes (v1 兼容) -----
    elif stage == 'combined-train' or stage == 'train':
        add_bprna('train', 'TR0')
        add_rnastr('train', 'tr')
    elif stage == 'combined-val' or stage == 'val':
        add_bprna('val', 'VL0')
        add_rnastr('val', 'vl')
    # ----- legacy aliases -----
...
class PriFoldSymFlowDataset(Dataset):
    def __init__(self, records, augment: bool = False,
                 select: float = 0.1, replace: float = 0.3,
                 limit: int | None = None):
...
def make_collate_fn(tokenizer, patch_size: int = 4, pos_bias_scale: float = 0.01):
    def collate(batch):
        ...
        tokenized = tokenizer.batch_encode_plus(
            seqs, padding='max_length', max_length=max_l + 2,
...
def build_loader(stage: str, config: dict, tokenizer, shuffle: bool):
    ...
    loader = DataLoader(
        ...
        collate_fn=make_collate_fn(
            tokenizer, config['model'].get('patch_size', 4),
            config['model'].get('pos_bias_scale', 0.01)),
