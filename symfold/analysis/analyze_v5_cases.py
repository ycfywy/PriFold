# -*- coding: utf-8 -*-
"""Per-RNA detailed case analysis for PriFold-SymFlow v5.

Analyzes every sample in bprna-test, outputs per-sample metrics,
and produces detailed breakdowns by length, density, pairing pattern, etc.

Usage:
  python symfold/analyze_v5_cases.py \
    --ckpt symfold/outputs/v5_bprna/model/best.pt \
    --out_dir symfold/outputs/v5_bprna/case_analysis
"""
...
from symfold.train.train_v5 import build_model, load_config, move_to_device
...
def per_sample_metrics(pred: torch.Tensor, target: torch.Tensor, length: int) -> dict:
    ...
    # Contact distance distribution (for GT)
    gt_dists = []
    pred_dists = []
...
def main():
    ...
    overall = {
        'n': len(all_rows),
        'f1': np.mean([r['f1'] for r in all_rows]),
        'precision': np.mean([r['precision'] for r in all_rows]),
        'recall': np.mean([r['recall'] for r in all_rows]),
        'pred_gt_ratio': np.mean([r['pred_gt_ratio'] for r in all_rows]),
        'density': np.mean([r['density'] for r in all_rows]),
        'length': np.mean([r['length'] for r in all_rows]),
    }
    ...
    bad_analysis = {
        ...
        'avg_f1': np.mean([r['f1'] for r in bad_cases]) if bad_cases else 0,
        'avg_length': np.mean([r['length'] for r in bad_cases]) if bad_cases else 0,
        'avg_density': np.mean([r['density'] for r in bad_cases]) if bad_cases else 0,
        'avg_pred_gt_ratio': np.mean([r['pred_gt_ratio'] for r in bad_cases]) if bad_cases else 0,
        'avg_gt_pairs': np.mean([r['gt_pairs'] for r in bad_cases]) if bad_cases else 0,
        'avg_pred_pairs': np.mean([r['pred_pairs'] for r in bad_cases]) if bad_cases else 0,
        'avg_mean_gt_dist': np.mean([r['mean_gt_dist'] for r in bad_cases]) if bad_cases else 0,
        'avg_max_gt_dist': np.mean([r['max_gt_dist'] for r in bad_cases]) if bad_cases else 0,
        'length_distribution': {},
    ...
    good_analysis = {
        ...
        'avg_f1': np.mean([r['f1'] for r in good_cases]) if good_cases else 0,
        'avg_length': np.mean([r['length'] for r in good_cases]) if good_cases else 0,
        'avg_density': np.mean([r['density'] for r in good_cases]) if good_cases else 0,
        'avg_pred_gt_ratio': np.mean([r['pred_gt_ratio'] for r in good_cases]) if good_cases else 0,
        'avg_mean_gt_dist': np.mean([r['mean_gt_dist'] for r in good_cases]) if good_cases else 0,
    }
