...
#   bash symfold/train/run_train.sh symfold/config/v4/v4_bprna.json
#   bash symfold/train/run_train.sh symfold/config/v3/v3_bprna.json
#   bash symfold/train/run_train.sh symfold/config/v2/v2_bprna_marsfix.json
#   bash symfold/train/run_train.sh symfold/config/v1/prifold_symflow_v1_full.json
#
# 行为：
#   1) 启动训练进程（独立进程组）
...
#   python -m symfold.analysis.show_gpu_stats symfold/outputs/<task>/gpu_stats.jsonl --tail 20
#   python -m symfold.analysis.show_gpu_stats symfold/outputs/<task>/gpu_stats.jsonl --summary
...
elif task.startswith('v11'):
    entry = 'symfold/train/train_v11.py'
elif version == 'v9' or task.startswith('v9_'):
    entry = 'symfold/train/train_v9.py'
elif version == 'v8' or task.startswith('v8_'):
    entry = 'symfold/train/train_v8.py'
elif version == 'v7' or task.startswith('v7_'):
    entry = 'symfold/train/train_v7.py'
elif version == 'v6' or task.startswith('v6_'):
    entry = 'symfold/train/train_v6.py'
elif version == 'v5' or task.startswith('v5_'):
    entry = 'symfold/train/train_v5.py'
elif version == 'v4' or task.startswith('v4_'):
    entry = 'symfold/train/train_v4.py'
elif version == 'v3' or task.startswith('v3_'):
    entry = 'symfold/train/train_v3.py'
else:
    entry = 'symfold/train/train_v2.py'
...
setsid bash -c "
  ...
  eval \"\$(/root/aigame/dannyyan/miniconda3/bin/conda shell.bash hook)\"
