...
from symfold.v3.model import PriFoldSymFlow_v2                    # noqa: E402
...
def _find_resume_boundary(history: list) -> int | None:
    """Find the epoch index where a resume-from-best occurred.
    Detected by 'resume_marker' flag or a large LR jump (>3x) between consecutive entries.
    Returns the index in history list (not epoch number) of the first resumed entry, or None.
    """
    for i, h in enumerate(history):
        if h.get('resume_marker'):
            return i
    # fallback: detect LR jumps
    for i in range(1, len(history)):
        prev_lr = history[i - 1].get('lr', 0)
        cur_lr = history[i].get('lr', 0)
        if prev_lr > 0 and cur_lr > prev_lr * 3:
            return i
    return None
...
def plot_curves(history: list, output_dir: Path, logger=None):
    ...
    # Detect resume boundary for dual-color plotting
    resume_idx = _find_resume_boundary(history)
    ...
    # Helper: split data into phase1 (before resume) and phase2 (after resume)
    def split_phases(xs, ys):
        """Split xs/ys into (phase1_x, phase1_y, phase2_x, phase2_y) based on resume epoch."""
        if resume_idx is None or not xs:
            return xs, ys, [], []
        resume_epoch = history[resume_idx]['epoch']
        p1x, p1y, p2x, p2y = [], [], [], []
        for x, y in zip(xs, ys):
            if x < resume_epoch:
                p1x.append(x); p1y.append(y)
            else:
                p2x.append(x); p2y.append(y)
        return p1x, p1y, p2x, p2y

    # Colors: phase1 = original, phase2 = distinguishable variant
    C_LOSS1, C_LOSS2 = '#d62728', '#ff7f0e'
    C_BCE1, C_BCE2 = '#ff9896', '#ffbb78'
    C_F11, C_F12 = '#1f77b4', '#17becf'
    C_MCC1, C_MCC2 = '#2ca02c', '#98df8a'
    C_P1, C_P2 = '#9467bd', '#c5b0d5'
    C_R1, C_R2 = '#8c564b', '#c49c94'
    C_LR1, C_LR2 = '#7f7f7f', '#ff7f0e'

    def plot_two_phase(ax, xs, ys, c1, c2, label, style='-o', ms=3, **kw):
        p1x, p1y, p2x, p2y = split_phases(xs, ys)
        if p1x:
            ax.plot(p1x, p1y, style, color=c1, label=label, ms=ms, **kw)
        if p2x:
            ax.plot(p2x, p2y, style, color=c2, label=f'{label} (resumed)', ms=ms, **kw)

    # ---- Training Loss ----
    ax = axes[0, 0]
    plot_two_phase(ax, epochs, train_loss, C_LOSS1, C_LOSS2, 'train loss')
    if any(b is not None for b in bce):
        plot_two_phase(ax, epochs, bce, C_BCE1, C_BCE2, 'bce', style='--', ms=0, lw=1)
    ...
    ax = axes[0, 1]
    if eval_epochs:
        plot_two_phase(ax, eval_epochs, val_f1, C_F11, C_F12, 'val F1', ms=4)
        plot_two_phase(ax, eval_epochs, val_mcc, C_MCC1, C_MCC2, 'val MCC', style='-s', ms=3)
    ...
    ax = axes[1, 0]
    if eval_epochs:
        plot_two_phase(ax, eval_epochs, val_p, C_P1, C_P2, 'val precision')
        plot_two_phase(ax, eval_epochs, val_r, C_R1, C_R2, 'val recall')
        plot_two_phase(ax, eval_epochs, val_f1, C_F11, C_F12, 'val F1')
    ax.set_title('Validation P / R / F1'); ax.set_xlabel('epoch'); ax.set_ylabel('score')
    ax.grid(alpha=0.3); ax.legend()

    # ---- Learning Rate ----
    ax = axes[1, 1]
    plot_two_phase(ax, epochs, lr, C_LR1, C_LR2, 'lr')
    ...
    if have_test:
        ...
        colors2 = ['#17becf', '#ff7f0e', '#98df8a', '#c5b0d5']
        for ti, stage in enumerate(test_stages):
            ...
            for h in history:
                ...
                if v is not None:
                    xs.append(h['epoch'])
                    ys.append(v)
            plot_two_phase(ax, xs, ys, colors[ti % len(colors)],
                           colors2[ti % len(colors2)], f'{stage} F1', ms=5)
        ...
        for ti, stage in enumerate(test_stages):
            ...
            for h in history:
                ...
                if v is not None:
                    ...
                    ys.append(v)
            plot_two_phase(ax, xs, ys, colors[ti % len(colors)],
                           colors2[ti % len(colors2)], f'{stage} MCC', style='-s', ms=4)
        ax.set_title('Test MCC (periodic eval)')
        ax.set_xlabel('epoch'); ax.set_ylabel('MCC')
        ax.grid(alpha=0.3); ax.legend()

    # Add resume boundary vertical line
    if resume_idx is not None:
        resume_epoch = history[resume_idx]['epoch']
        for row in range(n_rows):
            for col in range(2):
                axes[row, col].axvline(x=resume_epoch, color='red', linestyle='--',
                                       alpha=0.5, lw=1.5, label='_nolegend_')
...
def lr_for_epoch(config: dict, epoch: int, resume_epoch: int = 0) -> float:
    """Compute LR for given epoch.

    If resume_epoch > 0 (resume_from_best mode), we use a separate cosine schedule
    that starts from the LR at resume_epoch and decays over resume_cosine_epochs.
    This avoids the LR jump that occurs when total epochs config changes between runs.
    """
    ...
    if resume_epoch > 0 and epoch >= resume_epoch:
        # Resume mode: compute the LR that would have been at resume_epoch using the
        # *original* schedule (stored in config as resume_original_total, or default to
        # the LR value from the last history entry before resume).
        resume_start_lr = tcfg.get('resume_lr', None)
        if resume_start_lr is None:
            # Compute what LR was at resume_epoch-1 using original total
            orig_total = tcfg.get('resume_original_total', total)
            if resume_epoch - 1 < warmup:
                resume_start_lr = base_lr * resume_epoch / warmup
            else:
                prog = (resume_epoch - 1 - warmup) / max(orig_total - warmup, 1)
                resume_start_lr = base_lr * 0.5 * (1.0 + math.cos(math.pi * prog))

        resume_min_lr = tcfg.get('resume_min_lr', base_lr * 0.05)
        resume_cosine_epochs = tcfg.get('resume_cosine_epochs', 60)

        # No warmup for resumed training — start at resume_start_lr and cosine to min_lr
        elapsed = epoch - resume_epoch
        progress = min(elapsed / max(resume_cosine_epochs, 1), 1.0)
        return resume_min_lr + (resume_start_lr - resume_min_lr) * 0.5 * (1.0 + math.cos(math.pi * progress))
...
def main():
    ...
    logger.info('=' * 80)
    version = str(config.get('model', {}).get('version', 'v3'))
    logger.info(f"PriFold-SymFlow {version} training: {config['task_name']}")
    amp_on, amp_dtype = _resolve_amp_dtype(config)
    ...
    history = []
    best_f1 = -1.0
    _resume_epoch = 0  # >0 means we're in resume-from-best mode with separate LR schedule
    last_path = model_dir / 'last.pt'
    best_path = model_dir / 'best.pt'

    # Resume logic (checked in priority order):
    # A. If last.pt exists and was saved by a resume_from_best run (has resume_epoch>0),
    #    always auto-resume from it (handles mid-training interrupts).
    # B. If training.resume_from_best=true, load best.pt and start fresh cosine from best LR.
    # C. If last.pt exists + auto_resume=true, resume from last.pt (legacy behavior).
    # D. Otherwise, start from scratch.
    resume_from_best = config['training'].get('resume_from_best', False)

    # Check if last.pt is from a resumed-from-best run
    _last_is_resumed = False
    if last_path.exists():
        _ckpt_peek = torch.load(last_path, map_location='cpu', weights_only=False)
        _last_is_resumed = _ckpt_peek.get('resume_epoch', 0) > 0
        del _ckpt_peek

    if last_path.exists() and _last_is_resumed:
        # Case A: last.pt is from a resume_from_best run — always restore
        ...
        _resume_epoch = ckpt.get('resume_epoch', 0)
        logger.info(f'[Resume] from last.pt (resumed run): start_epoch={start_epoch} '
                    f'best_f1={best_f1:.4f} resume_epoch={_resume_epoch}')
    elif resume_from_best and best_path.exists():
        # Case B: Fresh resume from best checkpoint
        ckpt = torch.load(best_path, map_location=device, weights_only=False)
        model.load_state_dict(ckpt['model'])
        best_f1 = ckpt.get('best_f1', best_f1)
        start_epoch = ckpt.get('epoch', -1) + 1
        _resume_epoch = start_epoch
        # Load previous history from history.json (best.pt doesn't store full history)
        history_path = output_dir / 'history.json'
        if history_path.exists():
            full_history = json.loads(history_path.read_text())
            # Keep only entries up to (not including) start_epoch
            history = [h for h in full_history if h['epoch'] < start_epoch]
        # Optimizer freshly initialized — fine since resumed LR starts small
        logger.info(f'[ResumeFromBest] start_epoch={start_epoch} best_f1={best_f1:.4f} '
                    f'history={len(history)} entries, resume_epoch={_resume_epoch}')
    elif last_path.exists() and config['training'].get('auto_resume', True):
        # Case C: legacy auto_resume
        ckpt = torch.load(last_path, map_location=device, weights_only=False)
        model.load_state_dict(ckpt['model'])
        optimizer.load_state_dict(ckpt['optimizer'])
        history = ckpt.get('history', [])
        best_f1 = ckpt.get('best_f1', best_f1)
        start_epoch = ckpt.get('epoch', -1) + 1
        logger.info(f'[Resume] from last.pt: start_epoch={start_epoch} best_f1={best_f1:.4f}')
    else:
        # Case D: fresh start
        start_epoch = 0

    patience_count = 0
    # Mark the first epoch of a resumed run (for plot_curves dual-color)
    _is_first_resumed_epoch = (_resume_epoch > 0 and
                               not any(h.get('resume_marker') for h in history))
    for epoch in range(start_epoch, config['training'].get('epochs', 60)):
        lr = lr_for_epoch(config, epoch, resume_epoch=_resume_epoch)
        ...
        if _is_first_resumed_epoch:
            entry['resume_marker'] = True
            _is_first_resumed_epoch = False
