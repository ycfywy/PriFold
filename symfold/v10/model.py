# -*- coding: utf-8 -*-
"""PriFold v10: DensityNet-Ultra — Partial MARS Unfreeze + Enhanced Data.

v10 改进（基于 v9 评估 test F1=0.6961 的瓶颈分析）:
  U1. Partial MARS Unfreeze — 解冻最后 N 层 (默认2层)，让 LM 表征适配结构预测
  U2. Family-aware Curriculum — 低 F1 家族 oversampling + curriculum learning
  U3. 层级学习率 — MARS 层 lr=1e-5 (低), 下游头 lr=5e-4 (正常)

Architecture:
  RNA seq → MARS-LX (160M, 最后2层可训练) → 1D hidden + 2D attention
...
v9 → v10 对比:
  - 参数量: 5.09M → ~32M trainable (unfreeze 2 layers ≈ 27M extra)
  - 训练策略: 分层 LR + gradient clipping + warmup 更长
  - 数据: family-balanced sampling + hard sample mining
"""
...
# ============================================================
# 2D Rotary Position Embedding (RoPE) — 继承自 v9
# ============================================================
...
# ============================================================
# [U1] Partial MARS Unfreeze Strategy
# ============================================================

def partial_freeze_mars(extractor: nn.Module, unfreeze_last_n: int = 2,
                        unfreeze_norm: bool = True):
    """Freeze MARS layers except the last N transformer blocks.
    
    Strategy:
      - 冻结 embedding + 前 (12-N) 层 → 保留预训练知识
      - 解冻最后 N 层 → 适配下游结构预测任务
      - 解冻 final norm → 让表征尺度可调
      - MARS 层使用低 LR (1e-5)，下游头使用正常 LR (5e-4)
    
    Args:
        extractor: MARS model
        unfreeze_last_n: 解冻最后 N 层（默认 2，即 layer 11, 12）
        unfreeze_norm: 是否解冻 final norm
    """
    # 先冻结所有
    ...
    # 解冻最后 N 层
    n_layers = len(extractor.layers)
    for i in range(n_layers - unfreeze_last_n, n_layers):
        for p in extractor.layers[i].parameters():
            p.requires_grad = True
    
    # 解冻 final norm
    if unfreeze_norm and hasattr(extractor, 'norm'):
        for p in extractor.norm.parameters():
            p.requires_grad = True
    
    # 统计
    frozen = sum(p.numel() for p in extractor.parameters() if not p.requires_grad)
    unfrozen = sum(p.numel() for p in extractor.parameters() if p.requires_grad)
...
class DensityNetUltra(nn.Module):
    """v10: Partial MARS Unfreeze + Family-aware training.

    核心改进:
      U1. 解冻 MARS 最后 2 层 (~27M params)，允许 LM 特征适配任务
      U2. 训练中使用分层 LR: MARS 层 1e-5, 下游头 5e-4
      U3. 支持 family-balanced curriculum learning

    其余继承 v9 全部设计:
      - 2D RoPE 位置编码
      - Shift-aware margin loss
      - DST 低密度保护
      - 允许非标准配对
      - 增强正则化
    """
    ...
    def __init__(self,
                 extractor: nn.Module,
                 # [U1] MARS unfreeze config
                 freeze_mars: str = 'partial',  # 'full', 'partial', 'none'
        ...
        super().__init__()
        self.freeze_mars_mode = freeze_mars
        ...
        # [U1] MARS extractor with partial freeze
        self.extractor = extractor
        if freeze_mars == 'full':
            self.extractor.eval()
            for p in self.extractor.parameters():
                p.requires_grad = False
        elif freeze_mars == 'partial':
            frozen, unfrozen = partial_freeze_mars(
                self.extractor, unfreeze_last_n=unfreeze_last_n)
            print(f'[v10 MARS] Partial freeze: {frozen/1e6:.1f}M frozen, '
                  f'{unfrozen/1e6:.1f}M unfrozen (last {unfreeze_last_n} layers)')
        elif freeze_mars == 'none':
            pass  # All parameters trainable
        else:
            raise ValueError(f'Unknown freeze_mars mode: {freeze_mars}')

        # --- Feature Construction (same as v9) ---
        ...
        input_dim = hidden_dim + hidden_dim // 4 + hidden_dim // 8
        ...
        # --- Axial Transformer Stack with RoPE ---
    ...
    # ----------------------------------------------------------
    # [U1] Parameter group builder for layered LR
    # ----------------------------------------------------------

    def get_param_groups(self, mars_lr: float = 1e-5, head_lr: float = 5e-4,
                         weight_decay: float = 0.02):
        """Build parameter groups with different learning rates.
        
        Returns list suitable for torch.optim.AdamW:
          - Group 0: MARS unfrozen layers (low LR, no weight decay on norm/bias)
          - Group 1: MARS unfrozen layers (low LR, with weight decay)
          - Group 2: Downstream head (high LR, no weight decay on norm/bias)
          - Group 3: Downstream head (high LR, with weight decay)
        """
        mars_decay, mars_no_decay = [], []
        ...
        for name, param in self.named_parameters():
            ...
            if name.startswith('extractor.'):
                if 'norm' in name or 'bias' in name or param.ndim == 1:
                    mars_no_decay.append(param)
                else:
                    mars_decay.append(param)
            else:
                if 'norm' in name or 'bias' in name or param.ndim == 1:
        ...
        param_groups = [
            {'params': mars_no_decay, 'lr': mars_lr, 'weight_decay': 0.0, 'group_name': 'mars_no_decay'},
            {'params': mars_decay, 'lr': mars_lr, 'weight_decay': weight_decay, 'group_name': 'mars_decay'},
        ...
        for g in param_groups:
            ...
            print(f'  [{g["group_name"]}] {n_params/1e6:.2f}M params, lr={g["lr"]}, wd={g["weight_decay"]}')
    ...
    def _extract_mars(self, input_ids, attention_mask, set_len):
        if self.freeze_mars_mode == 'full':
            ...
            # [U1] Partial/no freeze: gradient flows through unfrozen layers
            hidden, attn_stack, hidden_layers = mars_forward_with_attn(
                self.extractor, input_ids, attention_mask,
                n_attn_layers=self.mars_n_attn_layers,
                hidden_layer_indices=self.mars_hidden_layer_indices,
    ...
    def _build_pair_features(self, mars_hidden, mars_attn, seq_oh):
        """Build pair features (same as v9)."""
    ...
    def _compute_loss(self, logit, contact, contact_mask, density_pred):
        ...
        if self.shift_loss_enabled:
            ...
            near_1 = fp_positions & (y_dilated_1 > 0.5)
            near_2 = fp_positions & (y_dilated_2 > 0.5) & (y_dilated_1 < 0.5)
            
            fp_bce = bce_raw * valid
            relief_1 = (fp_bce * near_1.float() * 0.7).sum()
            relief_2 = (fp_bce * near_2.float() * 0.4).sum()
