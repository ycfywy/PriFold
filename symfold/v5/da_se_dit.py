...
v5 upgrades v4:
  1. Larger model: hidden_dim 256→320, num_layers 9→12
  2. Deeper dilation pattern for better long-range modeling
  3. More aggressive triangle updates (start earlier at layer 4)
  4. Shared direct head with stop-gradient from flow path (less parameter waste)
"""
...
class DASEDiTBlockV5(nn.Module):
    """AdaLN-Zero × biased axial attention + triangle + SwiGLU.

    Same as V4 but with configurable pre-norm vs post-norm for stability at depth.
    """
...
class DASEDiT_MARS_v5(nn.Module):
    """v5 backbone: wider + deeper + stronger loss integration.

    Changes from v4:
      - hidden_dim: 256 → 320 (default)
      - num_layers: 9 → 12 (default)
      - dilation_pattern: 12 layers with progressive dilation
      - tri_start_layer: 6 → 4 (more triangle updates)
      - direct head shares final tokens but has own projection (lighter)
    """
    ...
    def __init__(self,
        ...
        if dilation_pattern is None:
            # 12 layers: progressive dilation for multi-scale
        ...
        # Input feature construction (same as v4)
        ...
        # Direct head (shares final_tokens, separate projection)
